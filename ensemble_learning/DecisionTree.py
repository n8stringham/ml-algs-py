'''
This file implements the Node and DecisionTree classes.
'''
import argparse
...
class DecisionTree():
    def __init__(self, metric=None, depth_limit=6, rf=False, rf_sample_size=2):
        assert metric is not None
        self.metric = metric
        self.depth_limit = depth_limit
        self.attr_vals = None
        self.rf_sample_size = rf_sample_size
        self.rf = rf
    ...
    def grow(self, S, attributes, depth):
        ...
        if depth <= self.depth_limit:
            ...
            #FIXME: Is this stopping condition necessary?
            #LATEX has the previous run
            #DesktopScreenshots are with this condition
            #if best_gain == 0:
            #    return Node(value=majority_label(S))
    ...
    def best_split(self, S, attributes):
        ...
        if self.rf: 
            attributes = random.sample(attributes, self.rf_sample_size)
            #print("attributes=",attributes)
    ...
    def predict_all(self, data):
        ...
        for instance in data:
            ...
            preds += self.predict(self.root, instance)
        return preds
